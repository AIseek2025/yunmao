...
1. `reports/work_report_iteration_21.md`
2. `reports/audit_report_iteration_21.md`
...
- `reports/work_report_iteration_22.md`
- 注意：Required Write-Back 中的目标 `work_report` 是本轮输出路径；如果该文件尚未落盘，禁止把它当作 Read First 输入读取。

## Source Audit Report

- `reports/audit_report_iteration_21.md`

## Audit Blocking Summary

- 说明 work report / 顶层 manifest / 实际物料说明 三者仍未统一
- 当前通过证据仍来自 brandos-MacBook-Pro-2.local 上的 local-ci
- 但顶层 manifest reports/iteration_21_artifact_sizes.txt 与 reports/iteration_21_artifact_sha256.txt 仍然包含 audit_payload_iteration_21.json
- Iteration 20 中关于证据一致性的主要问题，本轮只修了一半
