...
2. `.codemaster_orchestration/artifacts/opencode_session_stderr_phase_iter297_20260529120159.log`
...
- `previous_trace_id = phase_iter297_20260529120159`
- `previous_response_status = `
- `previous_returncode = `
- `previous_response_path = missing`
- `previous_stderr_path = .codemaster_orchestration/artifacts/opencode_session_stderr_phase_iter297_20260529120159.log`
- `attempt_count = 1`
...
- 若继续实现 App，新增写入必须优先落在  或其同目录子文件，禁止回到目录级 `repo`、`repo/apps`、`rules` 做泛读
- 若继续参考现有 workspace 配置，只允许按需读取 `go/go.work`；读完后必须直接进入实现或写回 `reports/work_report_iteration_297.md`
