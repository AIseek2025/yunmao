...
1. `reports/work_report_iteration_8.md`
2. `reports/audit_report_iteration_8.md`
...
- `reports/work_report_iteration_9.md`
- 注意：Required Write-Back 中的目标 `work_report` 是本轮输出路径；如果该文件尚未落盘，禁止把它当作 Read First 输入读取。

## Source Audit Report

- `reports/audit_report_iteration_8.md`

## Audit Blocking Summary

- GitHub Actions 未跑绿；
- PAT 缺少 workflow scope；
- gh CLI 未安装。
- 测试仍未在项目要求环境中实际通过。
- 本轮顶层 manifest 的原始摘录依旧只包含 7 个条目，并没有把 iteration_8_evidence/ 下所有原始证据逐条提升到顶层 manifest。
