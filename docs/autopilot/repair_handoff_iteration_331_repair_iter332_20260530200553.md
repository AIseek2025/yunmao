...
1. `reports/work_report_iteration_331.md`
2. `reports/audit_report_iteration_331.md`
...
- `reports/work_report_iteration_332.md`
- 注意：Required Write-Back 中的目标 `work_report` 是本轮输出路径；如果该文件尚未落盘，禁止把它当作 Read First 输入读取。

## Source Audit Report

- `reports/audit_report_iteration_331.md`

## Audit Blocking Summary

- Rust data-plane + JWT/JWKS 缺少 cross-service runtime/smoke/matrix 证据。
- TURN / RTC 缺少 /v1/rooms/{id}/ice-servers、credential mode、failure mode 的 round2 证据；
- Payments / IAP 缺少 round2 after 状态与 diagnostics evidence；
- 日志证据 /tmp/yunmao_iter331_cargo_test.log
- iteration 330 延续证据：
